...
{
	std::cout << "ClapTrap Default constructor called (Name is missingName)" << std::endl;
}
...
void ClapTrap::attack(const std::string& target)
{
	if (get_ep() != 0 && get_hp() != 0)
	{
		...
		std::cout << this->getName() << " ClapTrap is either dead or out of energy" << std::endl;
}
...
void ClapTrap::takeDamage(unsigned int amount)
{
	if (get_hp() == 0)
	{
		std::cout << getName() << " ClapTrap is already dead!" << std::endl;
		return ;	
	}
	if (this->_hp <= amount)
		_hp = 0;
